...
// store: K and pose
// compute: camera cods, pixel coods and world coods transformation
...
namespace myslam
{
  ...
  public:
    typedef std::shared_ptr<Camera> Ptr;   // define Ptr as camera pointer type, call ---Camera::Ptr
    float fx_, fy_, cx_, cy_, depth_scale_;// camera K      // use Sophus::SE3 to rep pose

    Camera();
    Camera( float fx, float fy, float cx, float cy, float depth_scale = 0):
        fx_ (fx), fy_(fy), cx_(cx), cy_(cy), depth_scale_(depth_scale)
    {}   // still donno the format meaning
